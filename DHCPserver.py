        ...
        try:
            ...
            if len(packet) > 270:
                ...
                if message_type == 1:
                    ...
                    new_packet = dhcp_offer(transaction_id, server_mac, client_mac, server_ip, offered_ip, mascara, router_ip, dns_ip)
                    s.sendto(new_packet, ('<broadcast>', 68))

                elif message_type == 3:
                    print(f'\nDHCP Request recebido de {addr}, enviando DHCP Acknowledgement')
                    new_packet = dhcp_ack(transaction_id, server_mac, client_mac, server_ip, offered_ip, mascara, router_ip, dns_ip)
                    s.sendto(new_packet, ('<broadcast>', 68))
